...
func TestBinaryAndBinaryPtrShortcuts(t *testing.T) {
	...
	// Ptr nil -> Skip.
	if f := field.BinaryPtr("k", nil); f.Type != ftype.Skip {
...
func TestByteStringAndPtrShortcuts(t *testing.T) {
	...
	if f := field.ByteStringPtr("k", nil); f.Type != ftype.Skip {
		t.Fatalf("ByteStringPtr(nil).Type = %v, want Skip", f.Type)
	}
	b2 := []byte("world")
	ptr := &b2
	f = field.ByteStringPtr("k", ptr)
	if f.Type != ftype.ByteString {
...
func TestStringAndPtrShortcuts(t *testing.T) {
	...
	if f := field.StringPtr("k", nil); f.Type != ftype.Skip {
		t.Fatalf("StringPtr(nil).Type = %v, want Skip", f.Type)
	}

	s := "x"
	f = field.StringPtr("k", &s)
...
func TestFloatShortcuts(t *testing.T) {
	...
	f32 := field.Float32("k", float32(2.5))
	...
	if f := field.Float64Ptr("k", nil); f.Type != ftype.Skip {
		t.Fatalf("Float64Ptr(nil).Type = %v, want Skip", f.Type)
	}
	if f := field.Float32Ptr("k", nil); f.Type != ftype.Skip {
...
func TestComplexShortcuts(t *testing.T) {
	...
	if f := field.Complex128Ptr("k", nil); f.Type != ftype.Skip {
		t.Fatalf("Complex128Ptr(nil).Type = %v, want Skip", f.Type)
	}
	if f := field.Complex64Ptr("k", nil); f.Type != ftype.Skip {
...
func TestTimeShortcuts(t *testing.T) {
	...
	// TimePtr nil -> Skip.
	if f := field.TimePtr("k", nil); f.Type != ftype.Skip {
	...
	f = field.TimeInLocation("k", tm2, loc)
	if f.Type != ftype.Time {
	...
	if f := field.TimeFullPtr("k", nil); f.Type != ftype.Skip {
