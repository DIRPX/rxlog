...
// Encoder encodes a logical name into the provided Buffer and returns the
// (possibly updated) Buffer pointer.
//
// An Encoder receives a destination buffer dst and a logical name string, and
// appends an encoded representation of that name to dst. Implementations
// SHOULD write into the existing buffer when possible to avoid unnecessary
...
//
// Implementations SHOULD document the chosen name encoding format (for
// example, raw string, quoted identifier, normalized or lowercased variant)
// and SHOULD keep this representation stable over time so that downstream
// systems can reliably match, group, or aggregate by name.
